...
@endif
  ...
  <?php $sno=1; ?>   
  {!! Form::open(['url'=>'repo/post/deleteselected','id'=>'multipledeleteform']) !!}
    ...
    <tbody>
      ...
      @foreach($posts as $key=>$post)
        <tr class="row-{{ $post->id }}">
          ...
          <td><a href="{{ url('repo/post/edit/'.$post->id) }}" class="btn btn-primary">Edit</a></td>
          <td>
          <!-- <a href="{{ url('repo_post.delete',$post->id) }}" data-method="delete" > Delete </a>x
...
<div id="myModal3" class="modal fade">
  <div class="modal-dialog modal-confirm">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-info" data-dismiss="modal">Cancel</button>
        <button type="button" class="btn btn-danger deletebtn" onclick="location.href='{{ url('repo/post/delete-all') }}'">Delete All</button>
      </div>
    </div>
  </div>
</div>   
